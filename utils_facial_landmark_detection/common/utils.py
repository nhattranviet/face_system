...
def drawLandmark(img, bbox, landmark):
    ...
    '''
    img_ = img.copy()
    cv2.rectangle(img_, (bbox.x1, bbox.x2), (bbox.y1, bbox.y2), (0,0,255), 2)
    for x, y in landmark:
...
def drawLandmark_Attribute(img, bbox, landmark,gender,age):
    ...
    - img marked with landmark and bbox
    '''
    cv2.rectangle(img, (bbox.x1, bbox.x2), (bbox.y1, bbox.y2), (0,0,255), 2)
    for x, y in landmark:
        cv2.circle(img, (int(x), int(y)), 3, (0,255,0), -1)
        if gender.argmax()==0:
                # -1->female, 1->male; -1->old, 1->young
                cv2.putText(img, 'female', (int(bbox.x1), int(bbox.x2)),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 3)
        else:
                cv2.putText(img, 'male', (int(bbox.x1), int(bbox.x2)),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0),3)
        if age.argmax()==0:
                cv2.putText(img, 'old', (int(bbox.y1), int(bbox.y2)),cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 0), 3)
        else:
                cv2.putText(img, 'young', (int(bbox.y1), int(bbox.y2)),cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 0), 3)
    return img
...
def check_bbox(img, bbox):
    ...
    if bbox.x > 0 and bbox.y > 0 and bbox.y1 < img_w and bbox.y2 < img_h:
        return True
    else:
        return False
...
def rotate(img, bbox, landmark, alpha):
    ...
    """
    center = ((bbox.x1+bbox.y1)/2, (bbox.x2+bbox.y2)/2)
    rot_mat = cv2.getRotationMatrix2D(center, alpha, 1)
    img_rotated_by_alpha = cv2.warpAffine(img, rot_mat, img.shape)
    landmark_ = np.asarray([(rot_mat[0][0]*x+rot_mat[0][1]*y+rot_mat[0][2],
                 rot_mat[1][0]*x+rot_mat[1][1]*y+rot_mat[1][2]) for (x, y) in landmark])
    face = img_rotated_by_alpha[bbox.x2:bbox.y2+1,bbox.x1:bbox.y1+1]
